-- Valeur déclarée assurée (somme des prix des articles cochés "Assurer cet
-- article") — distincte de insurance_cost (la prime de 0.6% payée par le
-- client) : c'est CETTE valeur qui doit être transmise à Sendcloud comme
-- insured_value du colis. Stockée à la confirmation plutôt que dérivée de
-- insurance_cost / taux au moment de l'étiquette, pour ne jamais dépendre
-- d'un taux qui pourrait changer après coup.
...
begin
  ...
  insert into public.orders (
    order_number, email, status, total_amount, subtotal, shipping_cost, insurance_cost, insured_value, currency,
    ...
    v_subtotal, coalesce(p_shipping_cost, 0), coalesce(p_insurance_cost, 0), coalesce(p_insured_value, 0), 'EUR',
...
revoke all on function public.confirm_b2b_payment(uuid, uuid[], jsonb, jsonb, text, text, text, uuid, numeric, uuid[], numeric, uuid, numeric) from public;
revoke all on function public.confirm_b2b_payment(uuid, uuid[], jsonb, jsonb, text, text, text, uuid, numeric, uuid[], numeric, uuid, numeric) from authenticated;
