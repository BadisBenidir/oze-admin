-- Commande groupée : le client peut rattacher une nouvelle commande à une
-- commande précédente déjà payée et pas encore expédiée, pour partager le
-- même envoi Sendcloud (livraison gratuite sur la seconde). Voir
-- useGroupableOrder.ts (recherche côté front) + b2b-checkout (validation et
-- calcul du tarif côté serveur, jamais de confiance sur le client).

alter table public.orders add column if not exists grouped_with_order_id uuid references public.orders (id);
create index if not exists idx_orders_grouped_with_order_id on public.orders (grouped_with_order_id);
...
revoke all on function public.confirm_b2b_payment(uuid, uuid[], jsonb, jsonb, text, text, text, uuid, numeric, uuid[], numeric, uuid) from public;
revoke all on function public.confirm_b2b_payment(uuid, uuid[], jsonb, jsonb, text, text, text, uuid, numeric, uuid[], numeric, uuid) from authenticated;
