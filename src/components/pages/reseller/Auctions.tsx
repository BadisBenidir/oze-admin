  ...
  return (
    <Card hover onClick={onOpen} className="overflow-hidden flex flex-col cursor-pointer">
      ...
      <div className="p-2.5 sm:p-4 flex-1 flex flex-col">
        {item.brand && <p className="text-[10px] sm:text-xs font-medium text-gray-500 truncate">{item.brand}</p>}
        <p className="text-xs sm:text-sm font-semibold text-gray-900 mt-0.5 line-clamp-2">{item.title}</p>

        <div className="flex items-start justify-between gap-1.5 mt-2 sm:mt-3">
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs text-gray-400">Prix actuel</p>
        ...
        {isWinning && (
          <div className="flex items-center gap-1 sm:gap-1.5 mt-1.5 sm:mt-2 text-green-700 bg-green-50 border border-green-100 rounded-lg px-1.5 sm:px-2.5 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium">
            <Trophy className="h-3 w-3 sm:h-3.5 sm:w-3.5 flex-shrink-0" />
            <span className="truncate">Vous menez{myMax ? ` (plafond : ${EUR(myMax)})` : ''}</span>
          </div>
        )}
        {!isWinning && isOutbid && (
          <div className="flex items-center gap-1 sm:gap-1.5 mt-1.5 sm:mt-2 text-red-700 bg-red-50 border border-red-100 rounded-lg px-1.5 sm:px-2.5 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium">
            <TrendingDown className="h-3 w-3 sm:h-3.5 sm:w-3.5 flex-shrink-0" />
            <span className="truncate">Surenchéri{myMax ? ` (plafond : ${EUR(myMax)})` : ''}</span>
          </div>
        )}
        ...
        {isPreview ? (
          ...
          <div className="mt-auto pt-2 sm:pt-3 space-y-2" onClick={(e) => e.stopPropagation()}>
            {/* Mobile (2 colonnes) : un seul bouton compact qui ouvre la fiche
                détail pour enchérir — la rangée de boutons rapides + le champ
                plafond libre n'auraient pas la place sur une carte à ~160px
                de large. */}
            <button
              onClick={onOpen}
              className="sm:hidden w-full py-1.5 px-1.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors text-xs font-medium"
            >
              Enchérir
            </button>
            ...
            {error && (
              <div className="flex items-center gap-1.5 text-red-600 text-[10px] sm:text-xs">
                <AlertCircle className="h-3.5 w-3.5 flex-shrink-0" />
                <span>{error}</span>
              </div>
            )}
            {!error && warning && (
              <div className="flex items-center gap-1.5 text-amber-600 text-[10px] sm:text-xs">
                <AlertCircle className="h-3.5 w-3.5 flex-shrink-0" />
                <span>{warning}</span>
              </div>
            )}
    ...
    <div className="p-4 md:p-6">
      ...
      {loading ? (
        <div className="grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-56 sm:h-96 bg-gray-100 rounded-lg animate-pulse" />)}
        </div>
      ) : !session ? (
