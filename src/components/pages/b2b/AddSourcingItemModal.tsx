...
export const AddSourcingItemModal: React.FC<AddSourcingItemModalProps> = ({ isOpen, onClose, onSubmit, onSubmitBatch, remainingCostBudget, existingProductIds }) => {
  ...
  // sourçables au même titre qu'un brouillon classique. Un article déjà
  // programmé dans un drop encore planifié passe en 'drop-b2b' (0135) dès son
  // ajout — donc naturellement exclu ici par ce même filtre de statut, sans
  // avoir besoin de vérifier séparément le product_ids des drops planifiés.
  useEffect(() => {
    if (!isOpen) return;
    let mounted = true;
